...
class WinnerGameCheck
{
    ...
    public function currentGameNumber($draw_id, $game_identifier, $date): int
    {
        ...
        $queryBuilder = $game->select(config('draw-engine.game_identifier_field'))
                             ->when($draw->frequency === 'day', function ($query) use ($date) {
                                 // Só contam os jogos do dia
                                 $query->whereDate('created_at', $date);
                             })
                             ->when($draw->frequency === 'week', function ($query) use ($date) {
                                 // Só contam os jogos do dia
                                 $query->where('week', $date->format('W'));
                             });
        // Só premiamos quem usa as Ream Codes
        $queryBuilder->whereRaw('code not like "___-__________-____"');
        $game_day_sequence = $queryBuilder->whereHas(config('draw-engine.region_field'),
            function ($query) use ($draw_id) {
                // Só contam o draw a que o game pertence
                $query->where('draw_id', $draw_id);
            })->orderBy('id', 'asc')->get();
        
        $game_day_sequence = $game_day_sequence->filter(function ($game) use ($game_identifier) {
            return $game[ config('draw-engine.game_identifier_field') ] === $game_identifier;
        })->keys()->first();
    ...
    private function burnGame($drawId, $game_identifier)
    {
        ...
        do {
            $newWinnerGameSequenceNumber = app(config('draw-engine.models.game'))->select('game')->whereDate('created_at',
                    now())->count() + random_int(5, 15);
